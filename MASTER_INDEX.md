# 📚 AICONTENT MASTER INDEX

**Purpose:** Central registry of ALL published content across Eden AI, Alpha News, and Second Brain.
**Search:** Use first 20 characters of title to find any post.

---

## 🧠 SECOND BRAIN (Andrey's Personal Agent)

| Date | Version | Description |
|------|---------|-------------|
| 2026-01-16 | v1.0 | Initial creation based on 40 onboarding questions |

---

## 🎬 EDEN AI (Video Content)

| Date | ID | Title (First 20 chars) | Folder | Platform | Status |
|------|----|-----------------------|--------|----------|--------|
| 2026-01-15 | EDEN_0115_GROK | პენტაგონის ახალი "გენერალი" | `Eden_AI/output/content/20260115_grok_pentagon/` | TikTok/YT | WIN |
| 2026-01-16 | EDEN_0116_MERGE | Merge Labs - ადამიანის ტვინის | `Eden_AI/output/content/20260116_merge_labs/` | TikTok/FB | NEUTRAL |
| 2026-01-17 | EDEN_0117_GROK_NUDES | გროკის "ნუდები" დასრულდა? | `Eden_AI/output/content/20260117_grok_nudes_lie/` | TikTok/FB | NEUTRAL |

---

## 📰 ALPHA NEWS (Posts)

| Date | ID | Title (First 20 chars) | Folder | Platform | Status |
|------|----|-----------------------|--------|----------|--------|
| *No entries yet* | | | | | |

---

## 🔍 HOW TO USE

1. User sends you first line of a post
2. Search this table by "Title (First 20 chars)"
3. Open the folder path to access all project files
4. Update results in `results.md` inside that folder

---

## 🤖 AGENTS IN THIS ECOSYSTEM

| Agent | Path | Purpose |
|-------|------|---------|
| **Alpha News** | `Alpha_News/.agent/skills/alpha_translator/` | Georgian AI news content |
| **Eden AI** | `Eden_AI/.agent/skills/viral_architect/` | Viral True Crime videos |
| **Second Brain** | `Second_Brain/.agent/skills/second_brain/` | Andrey's personal advisor |

